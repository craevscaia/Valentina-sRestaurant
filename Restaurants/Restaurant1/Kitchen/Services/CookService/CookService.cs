...
public class CookService : ICookService
{
    ...
    public async Task AddFoodToCookerList(Order order, IEnumerable<Food> foodList, Dictionary<int, List<Task>> tasks)
    {
        ...
        foreach (var food in foods.ToList())
        {
            switch (food.Complexity)
            {
                case 3:
                {
                    await PrepareFoodToGiveCooker(order, food, tasks, foods);
                    break;
                }
    ...
    // cooker for simple orders who can be fast done
    ...
    private async Task PrepareFoodToGiveCooker(Order order, Food food, Dictionary<int, List<Task>> tasks,
